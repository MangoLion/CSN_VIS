...
function findKNearestNeighbors(tree, querySegment, lineSegments, K) {
  ...
  for (const idx of nearestPoints) {
    const coords = [tree.points.get(idx, 0), tree.points.get(idx, 1), tree.points.get(idx, 2)];
  ...
  const searchRadius = maxRadius + 2 * lineSegments[0][0].sub(lineSegments[0][1]).norm().arraySync();
...
function generateRandomLineSegments(numSegments, C, boxSize) {
    ...
    const lineSegments = tf.transpose(tf.stack([startPoint, endPoint], 2), [0, 2, 1])
      .arraySync()
