...
const HugeCanvas = React.memo(
  ({
    ...
    function interpolateAlpha(target) {
      ...
      for (let i = 0; i < arr.length - 1; i++) {
        if (target >= arr[i].target && target <= arr[i + 1].target) {
          ...
          const proportion =
            (target - arr[i].target) / (arr[i + 1].target - arr[i].target);
    ...
    useEffect(() => {
      ...
      if (!manualUpdate && updateId != cid) updateView();
    ...
    const handleMouseDown = (e) => {
      ...
      if (snap) {
        let sl = streamLines;
        if (pixelMapData) sl = computeStreamlinesMap();
        sl.forEach((sl) => {
          if (xx >= sl[0] && xx <= sl[1]) xx = sl[0] - 1;
          if (yy >= sl[0] && yy <= sl[1]) yy = sl[0] - 1;
        });
      }
    ...
    const handleMouseMove = (e) => {
      ...
      let yy = Math.floor(pos.y * scaleX - layerRef.current.y() * scaleX);
      let ww = Math.floor(
        pos.x * scaleX - layerRef.current.x() * scaleX - selection.x
      );
      let hh = Math.floor(
        pos.y * scaleX - layerRef.current.y() * scaleX - selection.y
      );
    ...
    const updateSelection = (ss) => {
      ...
      const ranges = matrixSubregionIndexRanges(
        segments.length,
        ss.x,
        ss.y,
        ss.width,
      ...
      if (setGraphData) setGraphData(getGraphData());
    };
    ...
    function clampSelection(selection, bounds) {
      ...
      clamped.x = Math.max(
        bounds.x,
        Math.min(selection.x, bounds.x + bounds.width - selection.width)
      );

      // clamp y coordinate
      clamped.y = Math.max(
        bounds.y,
        Math.min(selection.y, bounds.y + bounds.height - selection.height)
      );

      // clamp width
      clamped.width = Math.min(
        selection.width,
        bounds.x + bounds.width - clamped.x
      );

      // clamp height
      clamped.height = Math.min(
        selection.height,
        bounds.y + bounds.height - clamped.y
      );
    ...
    function getEntryColor(x, y) {
      ...
      else colorScale = colorScaleGlobal;
    ...
    const computeStreamlinesMap = () => {
      ...
      for (let entry in pixelMapData) {
        ...
        if (!streamlinesMap[segments[idx].lineIDx]) {
          ...
          streamlinesMap[segments[idx].lineIDx][0] = Math.min(
            idx,
            streamlinesMap[segments[idx].lineIDx][0]
          );
          streamlinesMap[segments[idx].lineIDx][1] = Math.max(
            idx,
            streamlinesMap[segments[idx].lineIDx][1]
          );
        }
      }
    ...
    return (
      <div style={{ margin: "5px", height: "100%", width: "100%" }}>
          ...
          <div>
            ...
            <label>
              <input
                ...
                onChange={() => {
                  setDoSort(!doSort);
                  if (doSort) setStreamlines(streamLines2);
                }}
              />
              Sorted &nbsp;
              <select
                ...
                <div style={{ flexDirection: "column", width: "50%" }}>
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleAlgorithmChange2}
                    >
                      <option value="KNN">KNN</option>
                      <option value="RBN">RBN</option>
                    </select>
                  ...
                  <div>
                    <label>Distance:</label>
                    <select
                      defaultValue={distanceMetric2}
                      onChange={handleDistanceMetricChange2}
                    >
                      <option value="shortest">Shortest</option>
                      <option value="longest">Longest</option>
                      <option value="haustoff">Haustoff</option>
        ...
        <div ref={divRef} style={{ minHeight: "100vh", width: "100%" }}>
          <Stage
            ref={stageRef}
            style={{
