...
import { distance3D } from "./knnHelper";
...
const LineSegmentsComponent = ({
  ...
  const lineMeshes = useMemo(() => {
    ...
    for (let i = 0; i < segments.length; i++) {
      const startPoint = new THREE.Vector3(
        segments[i].startPoint[0],
        segments[i].startPoint[1],
        segments[i].startPoint[2]
      );
      const endPoint = new THREE.Vector3(
        segments[i].endPoint[0],
        segments[i].endPoint[1],
        segments[i].endPoint[2]
      );
      ...
      dummy.position.set(
        segments[i].midPoint[0],
        segments[i].midPoint[1],
        segments[i].midPoint[2]
      );

      const quaternion = new THREE.Quaternion().setFromAxisAngle(axis, angle);
      dummy.setRotationFromQuaternion(quaternion);

      const distance = distance3D(segments[i].startPoint, segments[i].endPoint);

      if (distance in counts) counts[distance]++;
      else counts[distance] = 1;
      ...
      mesh.setColorAt(i, new THREE.Color(segments[i].color));
    }
    return mesh;
  });
  ...
  //   return Object.entries(lineMap).map(
  //     ([lineIDx, { geometries, material }]) => {
  ...
  return (
    <group ref={groupRef}>
      {/*lineMeshes.map((mesh, index) => (<primitive key={index} object={mesh} />))*/}
      <primitive object={lineMeshes} />
    </group>
  );
};
